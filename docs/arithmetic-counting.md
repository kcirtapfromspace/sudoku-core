...
returns the reconstructed interval or divisibility contradiction. Cells are
...
- The deterministic search expands connected source groups, identifies equivalent
  ...
  nonzero coefficients. It reserves evaluation budget for deeper groups.
- `tested_combinations`, `budget_exhausted`, and `beam_pruned` describe the work
  ...
  this search is incomplete, and direct interval/gcd certificates themselves do
  not express every Boolean consequence.
- Certificate verification is independent of the search budget and node caches.
  ...
  most two. Invalid versions, IDs, weights, duplicates, targets, hashes, candidate
...
## Local acceptance results — 15 September 2026
